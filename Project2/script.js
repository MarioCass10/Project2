...
async function getTotalOwed(user) {
	...
	var monto_que_debo = 0;
	var monto_que_me_deben = 0;

	for (i = 0; i < listIOUs.length; i ++){

		if(user == listIOUs[i].debtor){
			monto_que_debo += parseInt(listIOUs[i].amount);
		}
		
		if(user == listIOUs[i].creditor){
			monto_que_me_deben += parseInt(listIOUs[i].amount);
		}
	}


	return monto_que_debo - monto_que_me_deben;
}
